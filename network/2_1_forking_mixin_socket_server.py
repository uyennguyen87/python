...
class ForkedClient():
    ...
    def run(self):
        ...
        print("Sent: %s characters, so far..." % send_data_length)

        # Display server response
        response = self.sock.recv(BUF_SIZE)
        print("PID %s received: %s"
              % (current_process_id, response.decode("ascii")[5:]))

    def shutdown(self):
        """ Cleanup teh client socket """
        self.sock.close()
...
class ForkingServerRequestHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        current_process_id = os.getpid()
        response = '%s: %s' % (current_process_id, data.decode("ascii"))
